...
def get_postfix(infix, n):
    ...
    # 문자열에서 하나씩 떼어와서 식을 만들자
    for i in range(n):
        # infix[i] => 중위표기식의 i 번째 글자
        if infix[i] not in "(+-*/)":
            # i 번째 글자가 피연산자다. => 결과에 출력
            postfix += infix[i]
        else:
            # i 번째 글자가 닫는 괄호인가?
            if infix[i] == ')':
                ...
                while stack:
                    ...
                    # 여는 괄호가 아니었다 => 출력
                    # 괄호 안에 있는 연산자는 최우선으로 계산되어야 한다.
                    postfix += op

            # i 번째 글자가 연산자다 => 우선순위 보고 판별
            else:
                # 현재 : - 이고 스택 안에는 : */ 라면 우선순위에 의하여
                # 현재 연산자의 우선순위보다 => icp[infix[i]]
                # 스택의 top 에 있는 연산자의 우선순위가 같거나 높으면 (isp[stack[-1]])
...
def get_result(postfix):
    ...
    # 계산이 잘 됐다면 스택 안에는 결과가 하나 남아있을 것이다.
    return stack.pop()
